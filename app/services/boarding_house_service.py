...
def translate_to_response(house: BoardingHouse):
    ...
    house_dict = {c.name: getattr(house, c.name) for c in house.__table__.columns if c.name != 'is_featured'}
    house_dict["images"] = [img.image_url for img in house.images]
    house_dict["is_featured"] = getattr(house, 'is_featured', False)
    house_dict["created_at"] = house.created_at
    house_dict["updated_at"] = house.updated_at
    
    ratings = [r.rating for r in house.reviews] if hasattr(house, 'reviews') else []
    house_dict["average_rating"] = sum(ratings) / len(ratings) if ratings else 0.0
    house_dict["review_count"] = len(ratings)
    house_dict["inquiry_count"] = len(house.inquiries) if hasattr(house, 'inquiries') else 0
    ...
    house_dict["reviews"] = [
        ...
        } for r in house.reviews
    ] if hasattr(house, 'reviews') else []
...
def get_all_boarding_houses(
    ...
    if search:
        ...
        query = query.filter(
            or_(
                BoardingHouse.title.ilike(search_filter),
                BoardingHouse.barangay.ilike(search_filter),
                BoardingHouse.city.ilike(search_filter)
            )
        )
    ...
    if gender:
        query = query.filter(BoardingHouse.gender_allowed.in_(gender))
...
def update_boarding_house_availability(db: Session, house_id: int, user_id: int, is_available: bool):
    """Surgically update the availability status of a boarding house."""
    house = db.query(BoardingHouse).filter(BoardingHouse.id == house_id).first()
